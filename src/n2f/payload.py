...
from business.constants import (
    AGRESSO_COL_EMAIL,
    AGRESSO_COL_FIRSTNAME,
    AGRESSO_COL_LASTNAME,
    AGRESSO_COL_ROLE,
    AGRESSO_COL_PROFILE,
    AGRESSO_COL_MANAGER,
...
def create_user_upsert_payload(user: dict, company_id: str, sandbox: bool) -> dict:
    ...
    payload = {
        ...
        N2F_COL_AUTH_MODE: (
            user[AGRESSO_COL_SSO_METHOD] if not sandbox else DEFAULT_AUTH_MODE_SANDBOX
        ),
    }
...
def create_project_upsert_payload(project: dict, sandbox: bool) -> dict:
    ...
    payload = {
        ...
        COL_NAMES: [
            {COL_CULTURE: CULTURE_FR, COL_VALUE: project[AGRESSO_COL_DESCRIPTION]},
            {COL_CULTURE: CULTURE_NL, COL_VALUE: project[AGRESSO_COL_DESCRIPTION]},
        ],
        N2F_COL_VALIDITY_DATE_FROM: normalize_date_for_payload(
            project.get(AGRESSO_COL_DATE_FROM)
        ),
        N2F_COL_VALIDITY_DATE_TO: normalize_date_for_payload(
            project.get(AGRESSO_COL_DATE_TO)
        ),
    }
