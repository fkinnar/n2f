...
class TestEndToEndIntegration(TestIntegrationBase):
    ...
    def test_full_synchronization_workflow(
        self,
...
class TestSynchronizerIntegration(TestIntegrationBase):
    ...
    def test_user_synchronizer_integration(self):
        ...
        test_entity = pd.Series(
            {"AdresseEmail": "test@example.com", "Nom": "Test User"}
        )
...
class TestPerformanceIntegration(TestIntegrationBase):
    ...
    def test_large_dataset_performance(
        ...
        success_result = SyncResult(
            "users", True, [large_df], error_message=None, duration_seconds=10.0
        )
        mock_executor.execute_scope.return_value = success_result
    ...
    def test_memory_usage_integration(
        ...
        # Mock des résultats
        result1 = SyncResult(
            "users", True, [df1], error_message=None, duration_seconds=2.0
        )
        result2 = SyncResult(
            "axes", True, [df2], error_message=None, duration_seconds=3.0
        )
        ...
        mock_cleanup_scope.side_effect = (
            lambda scope_name: mock_memory_manager.cleanup_scope(scope_name)
        )
...
class TestErrorRecoveryIntegration(TestIntegrationBase):
    ...
    def test_partial_failure_recovery(
        ...
        failure_result = SyncResult(
            "axes", False, [], error_message="API Error", duration_seconds=1.0
        )
